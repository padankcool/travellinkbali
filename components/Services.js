...
export default function Services() {
  ...
  return (
    <section 
      id="services" 
      // bg-fixed di sini adalah kunci dari efek Parallax CSS murni
      className="py-32 px-6 relative overflow-hidden bg-fixed bg-center bg-cover"
      style={{ 
        // Menggunakan gambar latar arsitektur resort tropis yang terang dan mewah
        backgroundImage: "url('https://images.unsplash.com/photo-1537996194471-e657df975ab4?q=80&w=2000&auto=format&fit=crop')" 
      }}
    >
      
      {/* Overlay Cerah agar teks terbaca, memberikan efek kaca (glassmorphism) yang bersih */}
      <div className="absolute inset-0 bg-white/90 backdrop-blur-[4px]" />

      <div className="max-w-7xl mx-auto relative z-10">
        
        {/* Header Section dengan Teks Gelap agar kontras dengan latar cerah */}
        ...
        {/* Grid Kartu Layanan - Kartu tetap mempertahankan gambar dengan gradient gelap agar menonjol (Pop Out) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {servicesList.map((service, index) => (
            <div 
              key={index}
              className="group relative h-[420px] rounded-3xl overflow-hidden border border-stone-200/50 shadow-2xl transition-all duration-500 hover:shadow-amber-500/20 hover:-translate-y-2"
            >
